-- MEM-085: Update default welcome template with improved onboarding content
-- Replaces the original template (id=1) with v3 that is self-contained,
-- platform-agnostic, and includes memory-sync download instructions.
...
**Prerequisites:** Node.js must be available. Check with `node --version`.

**Setup:**

1. Download the sync script to your project directory:
```
curl -o memory-sync.js https://llm-memory.net/tools/memory-sync.js
```
...
4. Run the sync:
```
node memory-sync.js --project-dir "<project-dir>" --config .agent.json --user <your-users-name>
```

This syncs notes bidirectionally between the API and local files. Add it to your session start procedure.

**If Node.js isn't available or you're not in a file-based environment**, skip this entirely. All note operations work through MCP tools — local sync is an optimization, not a requirement.
